#
# kimhokyeong_191205.R
#
# 김호경 2019.12.04 / 2019.12.05
# 
# 문1)
...
for( i in 1:ncol( st ) ) {
  out.val <- boxplot.stats( st[ , i ] )$out # 변수 이상치 vector
...
AQ2 <- AQ
for( i in 1:ncol( AQ2 ) ) {
  avg <- mean( AQ2[  , i ], na.rm = TRUE )    # 변수 평균값(결측치 제외)
  na.val <- AQ2[  , i ][ is.na( AQ2[ , i] ) ] # 변수 결측치 vector
  
  # 결측치가 있을 경우 결측치값만 평균값으로 치환
  if( length( na.val ) > 0 ) {
    AQ2[  , i ][ is.na( AQ2[ , i] ) ] <- avg
  }
}
AQ2
...
df[ order( df$Population ), ]

# (2) state.x77 데이터셋을 Income(소득)을 기준으로 내림차순 정렬하시오.
df[ order( df$Income, decreasing = TRUE ), ]

# (3) Illiteracy(문맹률)가 낮은 상위 10개 주의 이름과 문맹률을 출력하시오.
df.10 <- head( df[ order( df$Illiteracy ), ], 10 )
subset( df.10, select = c( "Illiteracy" ) )
